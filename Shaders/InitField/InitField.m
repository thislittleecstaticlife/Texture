...
//===------------------------------------------------------------------------===
#pragma mark - InitField Implementation
//===------------------------------------------------------------------------===

@implementation InitField
{
...
- (nullable instancetype)initWithLibrary:(nonnull id<MTLLibrary>)library
    ...
    if (nil != self) {
        ...
        id<MTLFunction> vertexFunction   = [library newFunctionWithName:@"init_field_vertex"];
        id<MTLFunction> fragmentFunction = [library newFunctionWithName:@"init_field_fragment"];
...
- (void)drawWithEncoder:(nonnull id<MTLRenderCommandEncoder>)renderEncoder
             fromBuffer:(nonnull id<MTLBuffer>)buffer
               atOffset:(NSInteger)fieldInitOffset
          instanceCount:(NSInteger)instanceCount {

    [renderEncoder setRenderPipelineState:pipelineState];
    [renderEncoder setVertexBuffer:buffer offset:fieldInitOffset atIndex:0];
